...
import '../../../services/database_market_place.dart';
import '../../../services/database_posts.dart';
import '../main_screen.dart';

class CreateMarketPostPage extends StatefulWidget {
  final String uid;

  CreateMarketPostPage({Key? key, required this.uid}) : super(key: key);

  @override
  _CreateMarketPostPageState createState() => _CreateMarketPostPageState();
}

class _CreateMarketPostPageState extends State<CreateMarketPostPage> {
  ...
  Future<String?> _uploadImage(File image) async {
    try {
      String filePath =
          'market_images/'+'${widget.uid}_${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  Future<void> createPost(String postText, String? imageUrl) async {
    ...
    await DatabaseService_market(uid: (widget.uid) + datetimenow)
        .updateUserData(name, postText, imageUrl,dp);

    await notification_service(uid: (widget.uid) + datetimenow)
        .updateUserData(name, "Added a market place item",dp);
  }
